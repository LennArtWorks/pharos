...
export const load = async () => {
  ...
  const topics = db.prepare('SELECT id, title, category_id, parent_id FROM topics ORDER BY title ASC').all() as any[];

  const allGlobalTopics = db.prepare(`
    SELECT id, title FROM topics WHERE category_id IN (SELECT id FROM categories WHERE is_global = 1)
  `).all() as any[];

  return { categories, topics, allGlobalTopics };
};
...
export const actions = {
  create: async ({ request }) => {
    ...
    // FIXED: Inherit category from parent if nested
    ...
    } else if (category_id === 'new_cat' && new_category_name) {
    ...
    try {
      db.prepare(`
        INSERT INTO topics (id, category_id, parent_id, title, human_description, prompt_text, files_list, components_list, design_tokens, ignored_parents, global_topics_list) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(id, category_id, parent_id === 'none' ? null : parent_id, title, human_description, prompt_text, files_list, components_list, design_tokens, ignored_parents, global_topics_list);
    } catch (err) {
      return fail(500, { error: 'Database error creating topic' });
    }

    throw redirect(303, `/dev/aidev/${id}`);
  }
};