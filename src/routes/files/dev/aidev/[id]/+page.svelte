	...
	<div class="bg-level-2 p-m pt-main-l border-border sticky top-0 z-10 flex items-center justify-between border-b">
		<div>
			{#if parentChain.length > 0}
				<div data-uiname="breadcrumbs" class="mb-1 flex flex-wrap items-center gap-2">
					{#each parentChain as parent}
						...
						{:else}
							<a
								href="/files/dev/aidev/{parent.id}"
			...
			{#if topic.human_description && !isEditing}
				<p class="text-body-m text-ink-50 mt-2 max-w-3xl">{topic.human_description}</p>
			{/if}
		</div>
					...
					<div class="flex flex-col gap-4">
						...
						<div class="flex flex-col gap-2">
							{#if deepContext.tokens.includes('colors')}
								<a href="/files/dev/design-system/colors" class="bg-level-1 border-border hover:border-accent-500 rounded-m flex items-center justify-between border p-3 transition-colors">
							...
							{#if deepContext.tokens.includes('icons')}
								<a href="/files/dev/design-system/icons" class="bg-level-1 border-border hover:border-accent-500 rounded-m flex items-center justify-between border p-3 transition-colors">
