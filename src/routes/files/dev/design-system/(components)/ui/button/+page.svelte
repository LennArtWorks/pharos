<script lang="ts">
	...
	// --- Sandbox State ---
	let variant = $state<'primary' | 'secondary' | 'tertiary' | 'template' | 'unstyled'>('primary');
	...
	function handleCodeInput(e: Event) {
		...
		if (['primary', 'secondary', 'tertiary', 'template', 'unstyled'].includes(parsedVariant)) {
			...
			<div data-uiname="controls-col-variant-size" class="flex flex-1 flex-col gap-6">
				<div data-uiname="control-group-variant" class="flex flex-col gap-2">
					...
					<div data-uiname="control-buttons-variant" class="flex flex-wrap gap-2">
						{#each ['primary', 'secondary', 'tertiary', 'template', 'unstyled'] as v}
